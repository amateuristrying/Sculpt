...
import { backendStatus, installRuntime, clearDownloadCache, importSource, readGeneratedAsset, detectHardware, generate, getEngines, saveGlb } from './harness';
import { parseGeneratedGlb, type ImportedAsset } from './geometry/importedAsset';
import type { SetupProgress, BackendStatus, EngineProfile, GeneratedAsset, GenerationProgress, HardwareProfile } from './harness/types';
...
export default function App() {
  ...
  const importImage = useCallback(async (file?: File) => {
    ...
    try {
      ...
      if (source?.sample && nativeSource && engines.some(engine => engine.id === 'triposr' && engine.compatibility !== 'unsupported')) setEngineId('triposr');
  ...
  const startGeneration = useCallback(async () => {
    if (!source || abortRef.current || installAbort.current || sourceLoadingRef.current || !engineId) return;
    const revision = sourceRevision.current;
    const controller = new AbortController(); abortRef.current = controller;
    setGenerating(true); setJobError(null); setProgress({ jobId: '', stage: 'analyzing', progress: 0, message: STAGES[0] });
    try {
      const result = await generate({ engineId, imageName: source.name, sourceId: source.sourceId, geometry: quality, background }, setProgress, controller.signal);
  ...
  }, [source, generating, engineId, quality, background, notify]);
  ...
  const resetProject = () => { clearSource(); setMode('material'); setNavigationMode('orbit'); setQuality('balanced'); setBackground('auto'); setMaterialColor(COLORS[0].value); setAutoRotate(false); setGrid(true); setLightIntensity(1); setTexture('None'); setEnabledStages({ cleanup: false, retopology: false, uv: false, texture: false }); setExportName('Sculpt — Loop study'); setResetKey((key) => key + 1); setModal(null); };
