...
import { detectHardware, generate, getEngines, saveGlb } from './harness';
import type { EngineProfile, GeneratedAsset, GenerationProgress, HardwareProfile } from './harness/types';

type SourceImage = { name: string; url: string; width: number; height: number; size: number; sample: boolean };
type Quality = 'draft' | 'balanced' | 'high';
type PanelTab = 'stack' | 'inspector';
type Modal = 'export' | 'hardware' | 'help' | 'new' | null;
const STAGES = ['Analyzing image', 'Generating geometry', 'Building surface', 'Preparing 3D asset'];
...
export default function App() {
  ...
  const [enabledStages, setEnabledStages] = useState<Record<string, boolean>>({ cleanup: false, retopology: false, uv: false, texture: false });
  const mesh = enabledStages.cleanup ? 'Optimized' : 'Original';
  ...
  const loadHardware = useCallback(async () => {
    ...
    try {
      ...
      const catalog = await getEngines(profile);
  ...
  const importImage = useCallback(async (file?: File) => {
    ...
    try {
      ...
      setSource({ name: file.name, url, width: image.naturalWidth, height: image.naturalHeight, size: file.size, sample: false });
      setAsset(null); setProgress(null); setExportName(file.name.replace(/\.[^.]+$/, ''));
    } catch { if (revision === sourceRevision.current) notify('This image could not be opened. Try another PNG, JPG, or WEBP.'); }
  ...
  const useSample = async () => {
    ...
    try {
      ...
      const image = new Image(); image.src = url; await image.decode();
      if (revision !== sourceRevision.current) return;
      setSource({ name: 'Loop study.png', url, width: image.naturalWidth, height: image.naturalHeight, size: Math.round(url.length * 0.75), sample: true }); setAsset(null); setProgress(null); setExportName('Sculpt — Loop study');
    } catch { if (revision === sourceRevision.current) notify('The preview is still preparing. Please try again.'); }
    finally { if (revision === sourceRevision.current) { sourceLoadingRef.current = false; setSourceLoading(false); } }
  };

  const startGeneration = useCallback(async () => {
    if (!source || abortRef.current || sourceLoadingRef.current || !engineId) return;
    const revision = sourceRevision.current;
    const controller = new AbortController(); abortRef.current = controller;
    setGenerating(true); setProgress({ jobId: '', stage: 'analyzing', progress: 0, message: STAGES[0] });
    try {
      const result = await generate({ engineId, imageName: source.name, geometry: quality }, setProgress, controller.signal);
      if (controller.signal.aborted || revision !== sourceRevision.current) return;
      setAsset(result); setResetKey((key) => key + 1); setPanelTab('stack'); notify('Your sample asset is ready. Orbit, inspect, and make it yours.');
    } catch (error) {
      ...
      else { setProgress(null); notify(error instanceof Error ? error.message : 'Generation failed. Please try again.'); }
    } finally { setGenerating(false); abortRef.current = null; }
  }, [source, generating, engineId, quality, notify]);

  const clearSource = () => { ++sourceRevision.current; sourceLoadingRef.current = false; setSourceLoading(false); setSource(null); setAsset(null); setProgress(null); };
  const resetProject = () => { clearSource(); setMode('material'); setNavigationMode('orbit'); setQuality('balanced'); setMaterialColor(COLORS[0].value); setAutoRotate(false); setGrid(true); setLightIntensity(1); setTexture('None'); setEnabledStages({ cleanup: false, retopology: false, uv: false, texture: false }); setExportName('Sculpt — Loop study'); setResetKey((key) => key + 1); setModal(null); };
  const exportAsset = async () => {
    if (!viewportApi.current || exporting) return;
    setExporting(true);
    try {
      const bytes = await viewportApi.current.exportGlb();
      const name = exportName.trim().replace(/[<>:"/\\|?*\u0000-\u001f]/g, '-').replace(/\.glb$/i, '') || 'Sculpt asset';
      const savedPath = await saveGlb(bytes, `${name}.glb`);
      if (savedPath) { setModal(null); notify(`GLB exported · ${Math.max(1, Math.round(bytes.byteLength / 1024)).toLocaleString()} KB · ${savedPath}`); }
  ...
  const stageIndex = progress ? ['analyzing', 'geometry', 'surface', 'preparing', 'complete'].indexOf(progress.stage) : 0;
    ...
    {!setupComplete ? <Setup hardware={hardware} engines={engines} selectedEngine={engineId} onSelect={setEngineId} onContinue={() => setSetupComplete(true)} error={setupError} onRetry={() => void loadHardware()} /> : <>
      ...
      <div className={`workspace ${focusViewport ? 'viewport-focused' : ''}`}>
        ...
        <main className="viewport-panel">
          <div className="viewport-topbar"><div className="viewport-title"><Box size={14} /><span>{asset ? 'Generated asset' : 'Loop study'}</span><span className="viewport-title-tag">{asset ? 'SIMULATED' : 'SAMPLE ASSET'}</span></div><div className="viewport-top-actions"><span>Perspective</span><button className={`icon-button ${focusViewport ? 'active' : ''}`} title="Focus viewport" aria-label="Focus viewport" onClick={() => setFocusViewport(!focusViewport)}><Maximize size={14} /></button></div></div>
          <div className="viewport-stage"><Viewport mode={mode} navigationMode={navigationMode} grid={grid || mode === 'technical'} autoRotate={autoRotate} lightIntensity={lightIntensity} materialColor={materialColor} resetKey={resetKey} assetSeed={asset?.seed ?? 0} geometryQuality={quality} onMetadata={onMetadata} onReady={onViewportReady} />
            ...
            {!asset && !generating && <div className="sample-hint"><span className="hint-line" /><p>A study in possibility.<span>{source ? 'Your source is ready. Generate to explore the workflow.' : 'Start with an image. Make it something more.'}</span></p></div>}
            {generating && <div className="generation-overlay" role="status" aria-live="polite"><div className="generation-orbit"><SculptMark size={29} /></div><span className="section-kicker">LOCAL GENERATION · SIMULATED</span><h2>{progress?.message || 'Preparing your asset'}</h2><div className="progress-track"><span style={{ width: `${progress?.progress || 0}%` }} /></div><div className="progress-stages">{STAGES.map((stage, index) => <div key={stage} className={index < stageIndex ? 'done' : index === stageIndex ? 'current' : ''}>{index < stageIndex ? <Check size={12} /> : index === stageIndex ? <LoaderCircle className="spin" size={12} /> : <Circle size={10} />}<span>{stage}</span></div>)}</div><button className="text-button" onClick={() => abortRef.current?.abort()}>Cancel</button></div>}
        ...
        <aside className="properties-panel"><div className="properties-tabs"><button className={panelTab === 'stack' ? 'active' : ''} onClick={() => setPanelTab('stack')}><Layers3 size={13} /> Sculpt stack</button><button className={panelTab === 'inspector' ? 'active' : ''} onClick={() => setPanelTab('inspector')}><SlidersHorizontal size={13} /> Inspector</button></div>
          <div className="properties-scroll">{panelTab === 'stack' ? <>
            ...
            <div className="stack-list"><StackRow number="01" icon={FileImage} label="Image" subtitle={source ? 'Source ready' : 'Waiting for source'} status={source ? 'complete' : 'idle'} /><StackRow number="02" icon={Box} label="Geometry" subtitle={generating ? 'Generating…' : asset ? 'Geometry generated' : 'Ready to generate'} status={generating ? 'running' : asset ? 'complete' : 'idle'} active /><StackRow number="03" icon={Sparkles} label="Mesh cleanup" subtitle="Remove & repair" status="planned" enabled={enabledStages.cleanup} onToggle={() => setEnabledStages((value) => ({ ...value, cleanup: !value.cleanup }))} /><StackRow number="04" icon={Grid2X2} label="Retopology" subtitle="Shape the structure" status="planned" enabled={enabledStages.retopology} onToggle={() => setEnabledStages((value) => ({ ...value, retopology: !value.retopology }))} /><StackRow number="05" icon={Expand} label="UV mapping" subtitle="Unfold the surface" status="planned" enabled={enabledStages.uv} onToggle={() => setEnabledStages((value) => ({ ...value, uv: !value.uv }))} /><StackRow number="06" icon={Layers3} label="Texture" subtitle="Add surface detail" status="planned" enabled={enabledStages.texture} onToggle={() => { const enabled = !enabledStages.texture; setEnabledStages((value) => ({ ...value, texture: enabled })); setTexture(enabled ? 'Color' : 'None'); }} /><StackRow number="07" icon={CircleDot} label="Material" subtitle={`${COLORS.find((color) => color.value === materialColor)?.name || 'Custom'} · preview`} status="preview" /><div className="stack-final"><Box size={15} /><span>Final asset</span><span className="tiny-pill">{asset ? 'SAMPLE READY' : 'PREVIEW'}</span></div></div><div className="stack-disclosure"><Info size={12} /><p>Processing stages are planned. Toggles save your pipeline choices for this session.</p></div>
            <div className="property-section"><div className="section-label">PROCESSING <span className="tiny-pill">PLANNED</span></div><label className="field-label">Mesh</label><Segments options={['Original', 'Optimized']} value={mesh} onChange={(value) => { setEnabledStages((stages) => ({ ...stages, cleanup: value === 'Optimized' })); }} /><label className="field-label spaced">Texture</label><Segments options={['None', 'Color', 'PBR']} value={texture} onChange={(value) => { setTexture(value); setEnabledStages((stages) => ({ ...stages, texture: value !== 'None' })); }} /></div>
