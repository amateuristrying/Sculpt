...
import { backendStatus, importSource, readGeneratedAsset, detectHardware, generate, getEngines, saveGlb } from './harness';
import { parseGeneratedGlb, type ImportedAsset } from './geometry/importedAsset';
import type { BackendStatus, EngineProfile, GeneratedAsset, GenerationProgress, HardwareProfile } from './harness/types';
...
export default function App() {
  ...
  const importImage = useCallback(async (file?: File) => {
    ...
    try {
      ...
      setAsset(null); setImportedAsset(null); setJobError(null); setProgress(null); setExportName(file.name.replace(/\.[^.]+$/, ''));
  ...
  const startGeneration = useCallback(async () => {
    ...
    try {
      const result = await generate({ engineId, imageName: source.name, sourceId: source.sourceId, geometry: quality }, setProgress, controller.signal);
  ...
  const clearSource = () => { ++sourceRevision.current; sourceLoadingRef.current = false; setSourceLoading(false); setSource(null); setAsset(null); setImportedAsset(null); setJobError(null); setProgress(null); };
  ...
  return <div className="app-shell">
    ...
    {!setupComplete ? <Setup backend={backend} hardware={hardware} engines={engines} selectedEngine={engineId} onSelect={setEngineId} onContinue={() => setSetupComplete(true)} error={setupError} onRetry={() => void loadHardware()} /> : <>
