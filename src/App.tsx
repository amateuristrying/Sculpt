...
import { backendStatus, installRuntime, clearDownloadCache, importSource, readGeneratedAsset, readSource, listGenerationJobs, getAccessStatus, detectHardware, generate, refine, getEngines, saveGlb, saveGeneratedGlb } from './harness';
import { parseGeneratedGlb, type ImportedAsset } from './geometry/importedAsset';
import type { RefinementSettings, AccessStatus, GenerationJob, SetupProgress, BackendStatus, EngineProfile, GeneratedAsset, GenerationProgress, HardwareProfile } from './harness/types';
...
type Modal = 'export' | 'hardware' | 'help' | 'new' | 'library' | null;
...
export default function App() {
  ...
  }, [source, reconstructionUnavailable, accessBlocked, engineId, quality, background, notify, refreshLibrary]);
  ...
  const openJob = async (job: GenerationJob) => {
    ...
    try {
      ...
      notify(sourceError ? 'Asset reopened. The original source image is missing or damaged; export is still available.' : job.asset ? 'Saved asset reopened from your local library.' : 'Source restored. You can retry generation.');
  ...
  return <div className="app-shell">
    ...
    {modal && <Dialog title={modal === 'library' ? 'Your local library.' : modal === 'export' ? 'Ready for the next step.' : modal === 'hardware' ? 'Your local environment.' : modal === 'new' ? 'Start a new project?' : 'Find your way around.'} onClose={() => { if (!exporting) setModal(null); }}>
