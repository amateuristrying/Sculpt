...
pub async fn import_source(
    ...
    let directory = app
        ...
        .join("sources");
    let record =
        tauri::async_runtime::spawn_blocking(move || decode_source(name, &data_url, &directory))
            .await
            .map_err(|e| e.to_string())??;
    ...
    harness
        ...
        .insert(asset.id.clone(), record);
    Ok(asset)
}

pub fn read_valid_glb(path: &Path) -> Result<Vec<u8>, String> {
    let length = std::fs::metadata(path)
        .map_err(|_| "The generated asset is missing")?
        .len();
...
pub fn validate_glb(bytes: &[u8]) -> Result<(), String> {
    ...
    {
        return Err("Generated GLB has no meshes".into());
    }
    ...
    if !has_binary {
        return Err("Generated GLB has no binary buffer".into());
    }
...
pub async fn read_generated_asset(
    ...
    tauri::async_runtime::spawn_blocking(move || {
        read_valid_glb(&path).map(|bytes| STANDARD.encode(bytes))
    })
    .await
    .map_err(|e| e.to_string())?
}
...
mod tests {
    ...
    fn rejects_empty_or_external_mesh_results() {
        ...
        let mut json = serde_json::to_vec(
            &serde_json::json!({"meshes":[{}], "buffers":[{"uri":"https://example.org/private"}]}),
        )
        .unwrap();
        while json.len() % 4 != 0 {
            json.push(b' ');
