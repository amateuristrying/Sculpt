...
def check_memory(quality, available_gb):
    ...
    if available_gb < needed:
        raise ValueError(f'Only {available_gb:.1f} GB of memory is currently available; {quality.title()} needs at least {needed:.1f} GB of headroom. Close other apps or choose Draft, then retry.')
    return profile
